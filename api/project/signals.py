...
# # Set up logging
# logger = logging.getLogger(__name__)
# updating_operation_team = False
# @receiver(post_save, sender=Project)
# def update_related_fields(sender, instance, **kwargs):
#     global updating_operation_team
#     if kwargs.get('raw', False) or updating_operation_team:
#         return

#     updating_operation_team = True

#     try:
#         operation_objects = ProjectUpdate.objects.filter(project_id=instance.id)
#         print(operation_objects)

#         if operation_objects.exists():
#             first_op_team = operation_objects.first()

#             if instance.sample:
#                 sample_increment = int(instance.sample) - (int(first_op_team.remaining_interview) + int(first_op_team.total_achievement))
#                 for op_team in operation_objects:
#                     op_team.remaining_interview = int(op_team.remaining_interview) + sample_increment
#                     op_team.save(update_fields=['remaining_interview'])

#             last_operation_team = operation_objects.last()
#             instance.remaining_interview = int(last_operation_team.remaining_interview)

#             aggregation_result = operation_objects.aggregate(
#                 total_man_days=Sum(Cast('total_man_days', FloatField())),
#                 total_achievement=Sum(Cast('total_achievement', IntegerField()))
#             )
#             instance.man_days = aggregation_result['total_man_days'] or 0
...
#     except Exception as e:
#         logger.error(f"An error occurred while updating operation team: {e}")

#     finally:
#         updating_operation_team = False
    
# @receiver(pre_save, sender=Project)
# def update_operation_team_end_date(sender, instance, **kwargs):
#     try:
#         # Check if this is an update and not a creation
#         if instance.pk:
#             old_instance = Project.objects.get(pk=instance.pk)
...
#                 # Update all related operationTeam objects
#                 operation_teams = ProjectUpdate.objects.filter(project_id=instance.id)
#                 for op_team in operation_teams:
#                     # Update remaining_time by adding the difference
#                     if op_team.remaining_time is not None:
#                         op_team.remaining_time += date_difference
#                     else:
#                         op_team.remaining_time = date_difference
#                     op_team.save()
#     except Project.DoesNotExist:
#         pass  # If the project does not exist, it is being created, so no need to update operationTeam here